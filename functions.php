...
if ( ! function_exists( 'notebook_setup' ) ) :
	...
	function notebook_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'notebook' ),
		) );
...
function notebook_scripts() {
	wp_enqueue_style( 'notebook-style', get_stylesheet_uri() );

	wp_enqueue_script( 'notebook-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'notebook-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
add_filter('get_search_form', 'notebook_search_button');

function notebook_search_button($text) {
